...
import Formulate from "../../packages/formulate/Formulate.js";
...
import { classKeys } from "../../src//libs/classes";

const globalRule = jest.fn((context) => {
  return false;
});
const options = {
  locales: {
...
describe("FormulateInput", () => {
  ...
  it("uses a hard-coded fallback validation error if no default rules exist", async () => {
    ...
    const wrapper = mount(FormulateInput, {
      localVue,
      propsData: { validation: "foobar", errorBehavior: "live" },
    });
    await flushPromises();
    expect(wrapper.find(".formulate-input-error").text()).toBe(
      "Invalid field value",
    );
    resetInstance();
  });
  ...
  it("allows custom field-rule level validation functions", async () => {
    const wrapper = mount(FormulateInput, {
      propsData: {
        ...
        validation: "required|in:abcdef",
        validationMessages: {
          in: ({ value }) => `The string ${value} is not correct.`,
        },
        errorBehavior: "live",
        value: "other value",
      },
  ...
  it("uses globally overridden validation messages", async () => {
    ...
    await flushPromises();
    expect(wrapper.find(".formulate-input-error").text()).toBe(
      "Super invalid email: wrong email",
    );
  });
  ...
  it("allows overriding the label default slot component", async () => {
    ...
    const wrapper = mount(FormulateInput, {
      localVue,
      propsData: { label: "My label here" },
    });
    expect(wrapper.find(".custom-label").html()).toBe(
      '<div class="custom-label">custom: My label here</div>',
    );
  });
  ...
  it("allows overriding the help default slot component", async () => {
    ...
    localVue.component("CustomHelp", {
      render: function (h) {
        return h("small", { class: "custom-help" }, [
          `custom: ${this.context.help}`,
        ]);
      },
      props: ["context"],
    });
    ...
    const wrapper = mount(FormulateInput, {
      localVue,
      propsData: { help: "My help here" },
    });
    expect(wrapper.find(".custom-help").html()).toBe(
      '<small class="custom-help">custom: My help here</small>',
    );
  });
    ...
    localVue.component("CustomErrors", {
      render: function (h) {
        return h(
          "ul",
          { class: "my-errors" },
          this.context.visibleValidationErrors.map((message) =>
            h("li", message),
          ),
        );
      },
  ...
  it("it allows overriding aria-describedby", async () => {
    // https://github.com/jest-community/eslint-plugin-jest/blob/v28.8.1/docs/rules/valid-title.md
    const wrapper = mount(FormulateInput, {
      propsData: {
        type: "text",
        validation: "required",
  ...
  it("continues to bail after content was entered", async () => {
    ...
    expect(wrapper.find(".formulate-input-errors li").text()).toBe(
      "Letters is required.",
    );
  });
  ...
  it("allows getFormValues inside of custom validation messages", async () => {
    ...
    expect(wrapper.find(".formulate-input-errors").exists()).toBe(true);
    expect(wrapper.find(".formulate-input-errors li").text()).toBe(
      "jon parker does not equal jon baley.",
    );
  });
  ...
  it("has formulate-input outer classes by default", () => {
    ...
    expect(wrapper.find(".formulate-input > *").attributes("class")).toBe(
  ...
  it("has formulate-input-label wrapper classes by default", () => {
    const wrapper = mount(FormulateInput, {
      propsData: { type: "text", label: "blah" },
    });
    ...
    localVue.use(Formulate, {
      baseClasses() {
        return classKeys.reduce(
          (classMap, key) => Object.assign(classMap, { [key]: "my-class" }),
          {},
        );
      },
    });
  ...
  it("can override individual classKey globals with functions", async () => {
    ...
    expect(wrapper.find(".formulate-input-wrapper").attributes("class")).toBe(
      "formulate-input-wrapper adds-2-class",
    );
    // Test the label override
    expect(wrapper.find("label").attributes("class")).toBe(
      "formulate-input-label formulate-input-label--before adds-3-class",
    );
    // Test the element override
    expect(wrapper.find(".formulate-input-element").attributes("class")).toBe(
      "formulate-input-element formulate-input-element--text adds-4-class",
    );
    // Test the input override
    expect(wrapper.find("input").attributes("class")).toBe("adds-5-class");
    // Test the input override
    expect(wrapper.find(".formulate-input-help").attributes("class")).toBe(
      "formulate-input-help formulate-input-help--after adds-6-class",
    );
    // Test the errors wrapper
    expect(wrapper.find(".formulate-input-errors").attributes("class")).toBe(
      "formulate-input-errors adds-7-class",
    );
    // Test the inner error
    expect(wrapper.find(".formulate-input-error").attributes("class")).toBe(
      "formulate-input-error adds-8-class",
    );
  });
  ...
  it("allows you to modify all class keys via props", async () => {
    ...
    // Test the wrapper override
    expect(wrapper.find(".formulate-input-wrapper").attributes("class")).toBe(
      "formulate-input-wrapper custom-2-class",
    );
    // Test the label override
    expect(wrapper.find("label").attributes("class")).toBe(
      "formulate-input-label formulate-input-label--before custom-3-class",
    );
    // Test the element override
    expect(wrapper.find(".formulate-input-element").attributes("class")).toBe(
      "formulate-input-element formulate-input-element--text custom-4-class",
    );
    // Test the input override
    expect(wrapper.find("input").attributes("class")).toBe("custom-5-class");
    // Test the input override
    expect(wrapper.find(".formulate-input-help").attributes("class")).toBe(
      "formulate-input-help formulate-input-help--after custom-6-class",
    );
    // Test the errors wrapper
    expect(wrapper.find(".formulate-input-errors").attributes("class")).toBe(
      "formulate-input-errors custom-7-class",
    );
    // Test the inner error
    expect(wrapper.find(".formulate-input-error").attributes("class")).toBe(
      "formulate-input-error custom-8-class",
    );
  });
  ...
  it("responds to hasValue state class key", async () => {
    ...
    await flushPromises();
    expect(wrapper.find("label").attributes("class")).toBe(
      "formulate-input-label formulate-input-label--before field-has-value",
    );
  });
  ...
  it("responds to globally registered hasValue on every element key", async () => {
    ...
    // Test the wrapper override
    expect(wrapper.find(".formulate-input-wrapper").attributes("class")).toBe(
      "formulate-input-wrapper has-2-value",
    );
    // Test the label override
    expect(wrapper.find("label").attributes("class")).toBe(
      "formulate-input-label formulate-input-label--before has-3-value",
    );
    // Test the element override
    expect(wrapper.find(".formulate-input-element").attributes("class")).toBe(
      "formulate-input-element formulate-input-element--text has-4-value",
    );
    // Test the input override
    expect(wrapper.find("input").attributes("class")).toBe("has-5-value");
    // Test the input override
    expect(wrapper.find(".formulate-input-help").attributes("class")).toBe(
      "formulate-input-help formulate-input-help--after has-6-value",
    );
    // Test the errors wrapper
    expect(wrapper.find(".formulate-input-errors").attributes("class")).toBe(
      "formulate-input-errors has-7-value",
    );
    // Test the inner error
    expect(wrapper.find(".formulate-input-error").attributes("class")).toBe(
      "formulate-input-error has-8-value",
    );
    resetInstance();
  });
  ...
  it("assigns isValid irregardless of error-behavior", async () => {
    ...
    await flushPromises();
    expect(wrapper.find("label").attributes("class")).toBe(
      "formulate-input-label formulate-input-label--before my-valid",
    );
  });
  ...
  it("extracts pseudoProps when camelCase", () => {
    const wrapper = mount(FormulateInput, {
      propsData: { type: "text", label: "blah", labelClass: "my-class" },
    });
  ...
  it("extracts pseudoProps when kebab case", () => {
    const wrapper = mount({
      template:
        '<FormulateInput type="text" label="blah" label-class="my-class" />',
    });
    expect(
      wrapper.findComponent(FormulateInput).vm.pseudoProps["labelClass"],
    ).toBe("my-class");
  });
  ...
  it("removes slotProps from the attributes and passes it", async () => {
    ...
    localVue.component("MyAddMore", {
      functional: true,
      render: (h, { props }) =>
        h("button", { class: "my-add-more" }, props.infoAddMore),
    });
    ...
    localVue.component("MyRemove", {
      functional: true,
      render: (h, { props }) =>
        h("div", { class: "my-remove" }, props.infoRemove),
    });
  ...
  it("allows an empty string as a validation prop", async () => {
    const wrapper = mount(FormulateInput, {
      propsData: { validation: "", errorBehavior: "live" },
    });
    await flushPromises();
    ...
    localVue.use(Formulate, {
      classes: {
        input: (context, classes) =>
          context.attrs.disabled ? classes.concat(["is-disabled"]) : classes,
      },
    });
    const wrapper = mount(FormulateInput, {
      localVue,
  ...
  it("allows custom slotProps for custom inputs", async () => {
    ...
    localVue.component("MyCustomInput", {
      functional: true,
      render: (h, { props }) =>
        h("button", { class: "my-custom-input" }, props.customInputProp),
    });
    localVue.use(Formulate, {
      library: {
        "my-input": {
  ...
  it("allows the disabling of input decorators with useInputDecorators: false", async () => {
    ...
    await flushPromises();
    expect(
      wrapper.find(".formulate-input-element-decorator").exists(),
    ).toBeFalsy();
    resetInstance();
  });
  ...
  it("can override the errorList slotComponent", async () => {
    ...
    localVue.component("CustomErrors", {
      render: function (h) {
        return h(
          "div",
          { class: "custom-errors" },
          this.visibleErrors.map((error) =>
            h("div", { class: "error-item", key: error }, error),
          ),
        );
      },
      props: ["visibleErrors"],
    ...
    expect(wrapper.find(".custom-errors .error-item").text()).toBe(
      "Name is required.",
    );
    resetInstance();
  });
  ...
  it("can override the prefix slotComponent", async () => {
    ...
    localVue.component("PrefixMe", {
      render: function (h) {
        return h("div", { class: "custom-prefix" }, [
          "Hello there",
          this.context.name,
        ]);
      },
    ...
    await flushPromises();
    expect(wrapper.find(".formulate-input-element .custom-prefix").text()).toBe(
      "Hello therejustin",
    );
    resetInstance();
  });
