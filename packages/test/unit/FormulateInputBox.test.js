...
import FormulateInputGroup from "../../src/inputs/FormulateInputGroup.vue";
...
describe("FormulateInputBox", () => {
  ...
  it("passes an explicitly given name prop through to the root radio elements", () => {
    const wrapper = mount(FormulateInput, {
      propsData: { type: "radio", name: "foo", options: { a: "1", b: "2" } },
    });
  ...
  it("passes an explicitly given name prop through to the root checkbox elements", () => {
    const wrapper = mount(FormulateInput, {
      propsData: { type: "checkbox", name: "foo", options: { a: "1", b: "2" } },
    });
    expect(wrapper.findAll('input[name="foo"]')).toHaveLength(2);
  });
  ...
  it("box inputs properly process options object in context library", () => {
    const wrapper = mount(FormulateInput, {
      propsData: { type: "checkbox", options: { a: "1", b: "2" } },
    });
  ...
  it('renders a group when type "radio" with options', () => {
    const wrapper = mount(FormulateInput, {
      propsData: { type: "radio", options: { a: "1", b: "2" } },
    });
    expect(wrapper.findComponent(FormulateInputGroup).exists()).toBe(true);
  });
  ...
  it('additional context does not bleed through to attributes with type "radio" and options', () => {
    ...
    expect(
      Object.keys(wrapper.find('input[type="radio"]').attributes()),
    ).toEqual(["type", "id", "value"]);
  });
  ...
  it('additional context does not bleed through to attributes with type "checkbox" and options', () => {
    ...
    expect(
      Object.keys(wrapper.find('input[type="checkbox"]').attributes()),
    ).toEqual(["type", "id", "value"]);
  });
  ...
  it("allows external attributes to make it down to the inner box elements", () => {
    ...
    expect(
      Object.keys(wrapper.find('input[type="radio"]').attributes()).includes(
        "readonly",
      ),
    ).toBe(true);
  });
  ...
  it("does not use the value attribute to be checked", () => {
    const wrapper = mount(FormulateInput, {
      propsData: { type: "checkbox", value: "123" },
    });
  ...
  it("uses the checked attribute to be checked", async () => {
    const wrapper = mount(FormulateInput, {
      propsData: { type: "checkbox", checked: "true" },
    });
    await flushPromises();
  ...
  it('uses the value attribute to select "type" radio when using options', async () => {
    const wrapper = mount(FormulateInput, {
      propsData: { type: "radio", options: { a: "1", b: "2" }, value: "b" },
    });
    await flushPromises();
  ...
  it("renders no boxes when options array is empty", async () => {
    const wrapper = mount(FormulateInput, {
      propsData: { type: "checkbox", options: [] },
    });
    expect(wrapper.findComponent(FormulateInputGroup).exists()).toBe(true);
  ...
  it("renders multiple labels both with correct id", async () => {
    const wrapper = mount(FormulateInput, {
      propsData: { type: "checkbox", label: "VueFormulate FTW!" },
    });
    const id = wrapper.find('input[type="checkbox"]').attributes("id");
    const labelIds = wrapper
      .findAll("label")
      .wrappers.map((label) => label.attributes("for"));
  ...
  it("has the proper default element classes when grouped", () => {
    ...
    expect(wrapper.findComponent(FormulateInputGroup).attributes("class")).toBe(
      "formulate-input-element formulate-input-element--group formulate-input-group extra",
    );
  });
  ...
  it("can add classes to the element wrapper", () => {
    ...
    expect(wrapper.findComponent(FormulateInputBox).attributes("class")).toBe(
      "formulate-input-element formulate-input-element--checkbox test-class",
    );
  });
  ...
  it("allows slot injection of of a prefix and suffix single checkbox", async () => {
    ...
    expect(wrapper.find(".formulate-input-element > span").text()).toBe(
      "$money",
    );
    expect(wrapper.find(".formulate-input-element > *:last-child").text()).toBe(
      "after money",
    );
  });
