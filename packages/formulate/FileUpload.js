...
class FileUpload {
  ...
  rehydrateFileList(items) {
    const fauxFileList = items.reduce((fileList, item) => {
      ...
      const url = item[key];
      const ext =
        url && url.includes(".") ? url.substr(url.lastIndexOf(".") + 1) : false;
  ...
  getUploader(...args) {
    if (this.uploaderIsAxios()) {
      ...
      return this.context.uploader
        .post(this.context.uploadUrl, formData, {
          headers: {
  ...
  removeFile(uuid) {
    ...
    if (
      window &&
      this.fileList instanceof FileList &&
  ...
  loadPreviews() {
    this.files.map((file) => {
      if (
        ...
        reader.onload = (e) =>
          Object.assign(file, { previewData: e.target.result });
  ...
  toString() {
    const descriptor = this.files.length
      ? this.files.length + " files"
      : "empty";
    return this.results
      ? JSON.stringify(this.results, null, "  ")
      : `FileUpload(${descriptor})`;
  }
}
