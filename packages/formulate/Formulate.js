...
class Formulate {
  ...
  typeProps(type) {
    const extract = (obj) =>
      Object.keys(obj).reduce((props, slot) => {
  ...
  slotProps(type, slot, typeProps) {
    let props = Array.isArray(this.options.slotProps[slot])
      ? this.options.slotProps[slot]
      : [];
