...
/**
 * Determine if the caller is running on the server side
 * @returns {boolean} true if server side, false otherwise
 */
export function isServerCall() {
...
export function camel(string) {
  if (typeof string === "string") {
    return string.replace(/([_-][a-z0-9])/gi, ($1) => {
      if (
        string.indexOf($1) !== 0 &&
        !/[_-]/.test(string[string.indexOf($1) - 1])
      ) {
...
export function groupBails(rules) {
  const groups = [];
  const bailIndex = rules.findIndex(
    ([, , rule]) => rule.toLowerCase() === "bail",
  );
  const optionalIndex = rules.findIndex(
  ...
  return groups.reduce((groups, group) => {
    ...
    const splitByMod = (group, bailGroup = false) => {
      ...
      if (modIndex >= 0) {
        ...
        splits.push(
          Object.defineProperty([group.shift()], "bail", { value: true }),
        );
...
export function regexForFormat(format) {
  ...
  return new RegExp(
    Object.keys(formats).reduce((regex, format) => {
 ...
 * @param {Array} keys of keys to extract
 */
